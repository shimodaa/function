...
def user_login(request):
    ...
    if login_form.is_valid():
        ...
        if user:
            if user.is_active:
                login(request, user)
                messages.success(request, 'ログイン完了しまちた。')
                return redirect('accounts:home')
            else:
                messages.warning(request, 'ユーザがアクティブではありまてん')
        else:
            messages.warning(request, 'ユーザかパスワードが間違っとります')
    return render(
...
def user_logout(request):
    logout(request)
    messages.success(request, 'ログアウトしまちた')
    return redirect('accounts:home')
